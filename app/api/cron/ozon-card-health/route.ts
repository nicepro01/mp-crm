...
// См. app/api/cron/ozon-unit-economics/route.ts — один из отдельных ночных
// cron-вызовов для Ozon, разбитых из-за лимита времени Vercel Hobby.
...
export async function GET(req: NextRequest) {
  ...
  for (const company of companies) {
    ...
    try {
      results[company.id] = await runWithTenant({ companyId: company.id, userId }, runOzonCardHealthSync);
    } catch (err: any) {
