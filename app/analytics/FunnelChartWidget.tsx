...
const LIMITATION_TEXT: Record<string, string> = {
  WB: "История ограничена ~30 днями — реальный API Wildberries не отдаёт заказы старше месяца. Данные копятся вперёд по мере регулярных синков: чем чаще нажимаете «Обновить», тем длиннее становится история. Последние ~10 дней помечены звёздочкой (*) — исход заказа (выкуп/отказ) там ещё не наступил. В режиме «По месяцам» звёздочка также означает неполный месяц — учтены только те дни, что успели накопиться, а не весь месяц целиком.",
  OZON: "Сумма заказа (₽) — новая, не проверенная на реальном аккаунте часть (штуки и статусы уже подтверждены). Если суммы выглядят неправдоподобно — сообщите, поправим. В режиме «По месяцам» звёздочка (*) означает неполный месяц — учтены только накопленные дни, а не весь месяц целиком.",
...
// Те же короткие названия месяцев, что и в app/analytics/page.tsx
// (MONTH_NAMES_SHORT) — toLocaleDateString с year:"2-digit" добавляет "г."
// после года ("июнь 26 г."), не нужно.
