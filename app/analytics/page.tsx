...
async function AnalyticsPageContent() {
  ...
  function marginFields(productId: string, marketplaceId: string) {
    ...
    return {
      ...
      // невыкуплено); Ozon: отправления старше лага со статусом delivered ÷
      // (delivered + cancelled) по /v3/posting/{fbo,fbs}/list — см.
      // sync-wb/sync-ozon/sync-yandex), не оценка.
  ...
  const perMarketplaceColumns: SortableColumn[] = [
    ...
    { key: "buybackPct", label: "% выкупа", type: "number", description: "Доля заказов, которые покупатель реально забрал и оплатил (не отменил/не отказался на ПВЗ) — реальный сигнал с площадки, не оценка. Пусто — ещё не прошёл лаг ожидания исхода (10 дней) или совсем не было заказов", width: 48 },
