...
export default function OzonApiSyncForm() {
  ...
  async function handleSync() {
    ...
    const res = await fetch("/api/stock-import/ozon-sync", { method: "POST" });
    setSyncing(false);
  ...
  return (
    <div style={{ maxWidth: 640, marginBottom: 24 }}>
      <div style={{ marginBottom: 8 }}>
        <strong>Синхронизация через Ozon API</strong> — остатки на складах FBO,
        без ручной загрузки файла.
      </div>
      <button type="button" className="btn" onClick={handleSync} disabled={syncing}>
        {syncing ? "Синхронизация…" : "Обновить из Ozon API"}
      </button>
