...
const columns: { key: SortKey; label: string; type: "string" | "number"; description: string; width: number }[] = [
  ...
  },
  {
    ...
    type: "number",
    description:
      "WB и Ozon: доля реальных заказов, которые НЕ были отменены/не выкуплены за последние 30 дней (заказы младше 10 дней не считаются — исход ещё не известен). Считается по факту заказов, а не по продажам/возвратам",
    width: 52,
  },
  {
    key: "returnsQty",
